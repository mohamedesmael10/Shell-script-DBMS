...
if ((field_no == 0)); then echo "Error : Invalid Column Name (・_・;)"; return; fi
...
if ((found == 0)); then echo "Error : Invalid Column Name (・_・;)"; return; fi
...
if ((record_field == 0)); then echo "Error : No Value Found (・_・;)"; return; fi

#validate the new value with the column type
new_value=$(echo "$sql_line" | awk -F';' '{print $2}' | awk -F'=' '{gsub(/^[ \t]+|[ \t]+$/, "",$2);print $2}')
case $new_value in
[a-zA-Z]*) if ((col_data_type != "txt")); then echo "Error : Invalid Value Type (・_・;)";return; fi ;;
[0-9]*) if ((col_data_type != "int")); then echo "Error : Invalid Value Type (・_・;)";return; fi ;;
*) exit 4 ;;
esac
...
if ((is_pk == "pk")); then
        ...
        if ((value_exist == 1)); then echo "Error : PK Value Exists (・_・;)";return; fi
fi
